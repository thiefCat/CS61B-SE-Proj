...
public class TERenderer {
    ...
    public void drawMenu(int state, String seed, String name) {
        StdDraw.clear(Color.BLACK);
        StdDraw.setPenColor(Color.WHITE);
        int sizeBig = height;
        int sizeSmall = height * 2 / 3;
        Font fontBig = new Font("Monaco", Font.BOLD, sizeBig);
        Font fontSmall = new Font("Monaco", Font.BOLD, sizeSmall);
        StdDraw.setFont(fontBig);
        StdDraw.text(this.width / 2, this.height * 2 / 3, "CS61B: THE GAME");
        StdDraw.setFont(fontSmall);
        StdDraw.text(this.width / 2, this.height * 1 / 2, "New Game (N)");
        StdDraw.text(this.width / 2, this.height * 1 / 2 - 2, "Load Game (L)");
        StdDraw.text(this.width / 2, this.height * 1 / 2 - 4, "Rename (R)");
        StdDraw.text(this.width / 2, this.height * 1 / 2 - 6, "Quit (Q)");
        if (state == Engine.HANDLE_SEED) {
            StdDraw.text(this.width / 2, this.height * 1 / 2 - 10, "Enter Seed: " + seed);
        } else if (state == Engine.HANDLE_RENAME) {
            StdDraw.text(this.width / 2, this.height * 1 / 2 - 10, "Set Your Name: " + name);
        }
        StdDraw.show();
        Font font = new Font("Monaco", Font.BOLD, TILE_SIZE - 2);
        StdDraw.setFont(font);
